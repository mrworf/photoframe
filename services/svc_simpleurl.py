...
class SimpleUrl(BaseService):
    ...
    def helpKeywords(self):
        return 'Each item is a URL that should return a single image. ' \
               'The URL may contain the terms "{width}" and/or "{height}" which will be ' \
               'replaced by numbers describing the size of the display.'
    ...
    def selectImageFromAlbum(self, destinationDir, supportedMimeTypes, displaySize, randomize, retry=1):
        ...
        if result.error is not None and result.source is not None:
            logging.warning("broken url detected. You should remove '.../%s' from keywords" %
        ...
        # catch unsupported mimetypes (can only be done after downloading the image)
            ...
            logging.warning("unsupported mimetype '%s'. You should remove '.../%s' from keywords" %
