...
class drivers:
    ...
    def list(self):
        result = {}
        if os.path.exists(path.DRV_BUILTIN):
            result = self._list_dir(path.DRV_BUILTIN)
        # Any driver defined in external that has the same name as an internal one
        # will replace the internal one.
        result.update(self._list_dir(path.DRV_EXTERNAL))
            ...
            with open(installer, 'r') as f:
                for line in f:
                    ...
                    if state == 1:
                        ...
                        if dst == '' or src == '':
                            logging.error(
                                'Install section cannot have an empty source or destination filename (Line %d)', lc)
                            return None
                        ...
                        if key in config['options']:
                            logging.warning(
                                'Key "%s" will be overridden since it is defined multiple times (Line %d)', lc)
