...
class display:
    ...
    def get(self):
        ...
        if not self.enabled:
            ...
            with open(self.getDevice(), 'rb') as fb:
                src = subprocess.Popen(['/root/photoframe/rgb565/rgb565', 'reverse'],
                                       stdout=subprocess.PIPE, stdin=fb, stderr=self.void)
            ...
            if self.isHDMI():
                if force:  # Make sure display is ON and set to our preference
                    if os.path.exists('/opt/vc/bin/tvservice'):
                        debug.subprocess_call(['/opt/vc/bin/tvservice', '-e', self.params],
    ...
    def available():
        if os.path.exists('/opt/vc/bin/tvservice'):
            cea = json.loads(debug.subprocess_check_output(
                ['/opt/vc/bin/tvservice', '-j', '-m', 'CEA'], stderr=subprocess.STDOUT))
            dmt = json.loads(debug.subprocess_check_output(
                ['/opt/vc/bin/tvservice', '-j', '-m', 'DMT'], stderr=subprocess.STDOUT))
        else:
