    ...
    def presentation(self):
        ...
        while self.running:
            ...
            displaySize = {'width': self.settings.getUser('width'), 'height': self.settings.getUser(
                'height'), 'force_orientation': self.settings.getUser('force_orientation')}
            randomize = self.settings.getUser('randomize_images')

            try:
                if self.historyIndex == -1:
                    result = self.services.servicePrepareNextItem(self.settings.get(
                        'tempfolder'), self.supportedFormats, displaySize, randomize)
            ...
            time_process = time.time() - time_process
            logging.debug('Took %f seconds to process, next image is %s', time_process,
                          result.filename if result is not None else "None")
