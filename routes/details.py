...
class RouteDetails(BaseRoute):
    ...
    def handle(self, app, about):
        if about == 'tvservice':
            ...
            output = subprocess.check_output(['git', 'log', '-n1'], stderr=self.void).decode('utf-8')
            lines = output.split('\n')
            infoDate = lines[2][5:].strip()
            infoCommit = lines[0][7:].strip()
            output = subprocess.check_output(['git', 'status'], stderr=self.void).decode('utf-8')
            lines = output.split('\n')
            ...
            output = ''
            try:
                output = subprocess.check_output(['/opt/vc/bin/vcgencmd', 'get_throttled'], stderr=self.void).decode('utf-8')
            except Exception as e:
                logging.debug('Unable to execute /opt/vc/bin/vcgencmd, assuming not running on Raspberry Pi')
            ...
            if timeavailable > 0 and timeneeded > timeavailable:
                msgs.append({
                    'level': 'WARNING',
                    'message': f'Change every {self.settings.getUser("interval")} seconds with {images} images will take {timeneeded/3600}h, refresh keywords is {timeavailable/3600}h',
                    'link': None
                })
